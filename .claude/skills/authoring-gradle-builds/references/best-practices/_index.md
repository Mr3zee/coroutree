...
Generated from Gradle 9.7.0 documentation.
...
- [Always Declare Attributes on Consumable and Resolvable Configurations](always-declare-attributes-on-consumable-and-resolvable-configurations.md) — When creating custom configurations that are meant to be consumed or resolved, always declare at least one [attribute on them. `#dependencies`
...
- [Consider use of `@Incubating` APIs carefully](consider-use-of-incubating-apis-carefully.md) — Use Incubating APIs deliberately and with awareness of trade-offs. New or unstable Incubating features can require updates even on minor Gradle upgrades, while… `#upgrades`
...
- [Validate the Gradle Distribution SHA-256 Checksum](validate-the-gradle-distribution-sha-256-checksum.md) — Set `distributionSha256Sum` in `gradle-wrapper.properties` to verify the integrity of the downloaded Gradle distribution. `#properties` `#wrapper`
- [Validate the Gradle Wrapper on every Upgrade](validate-the-gradle-wrapper-on-every-upgrade.md) — The Gradle Wrapper runs before your build logic and can deeply affect your build. You should treat any change to the Wrapper as security-sensitive. Validate… `#wrapper`
- [Do not Run `./gradlew` on Untrusted Projects](do-not-run-gradlew-on-untrusted-projects.md) — When working with an untrusted project, you should exercise caution before running `./gradlew`. `#wrapper`
- [Build Output Should Be Byte-for-Byte Reproducible](build-output-should-be-byte-for-byte-reproducible.md) — A build is reproducible when the same source code produces byte-for-byte identical output files, on any machine, at any time. `#publishing` `#tasks`
...
- [Don't hardcode Task names unless they are documented as Public API](don-t-hardcode-task-names-unless-they-are-documented-as-public-api.md) — Hardcoding task names can make a build fragile when upgrading Gradle or third-party plugins. `#tasks` `#upgrades`
- [Don't access a `Project` instance during Task Execution](don-t-access-a-project-instance-during-task-execution.md) — Do not access a `Project` instance inside a task action or task property. `#tasks` `#inputs-and-outputs`
- [Wire lazy task outputs using `map` and `flatMap`](wire-lazy-task-outputs-using-map-and-flatmap.md) — Use `flatMap` to extract a `Provider`-typed output from a task, and `map` to transform the resulting value. Together, they preserve the task dependency chain… `#tasks` `#task-configuration-avoidance`
...
- `#dependencies` — don-t-explicitly-depend-on-the-kotlin-standard-library, avoid-redundant-dependency-declarations, declare-dependencies-using-a-single-gav-group-artifact-version-string, use-content-filtering-with-multiple-repositories, apply-exclusions-narrowly, always-declare-attributes-on-consumable-and-resolvable-configurations
- `#inputs-and-outputs` — avoid-dependson, do-not-call-get-on-a-provider-outside-a-task-action, avoid-using-eager-apis-on-file-collections, don-t-resolve-configurations-before-task-execution, use-pathsensitivity-none-for-file-inputs-and-pathsensitivity-relative-for-directories, use-unique-output-files-and-directories, don-t-access-a-project-instance-during-task-execution
...
- `#properties` — set-build-flags-in-gradle-properties, do-not-use-gradle-properties-in-subprojects, use-utf-8-file-encoding, use-the-build-cache, use-the-configuration-cache, validate-the-gradle-distribution-sha-256-checksum
- `#publishing` — build-output-should-be-byte-for-byte-reproducible
...
- `#task-configuration-avoidance` — avoid-afterevaluate, wire-lazy-task-outputs-using-map-and-flatmap
- `#tasks` — avoid-expensive-computations-in-configuration-phase, build-output-should-be-byte-for-byte-reproducible, avoid-dependson, favor-cacheabletask-and-disablecachingbydefault-over-cacheif-spec-and-donotcacheif-string-spec, do-not-call-get-on-a-provider-outside-a-task-action, group-and-describe-custom-tasks, avoid-using-eager-apis-on-file-collections, don-t-resolve-configurations-before-task-execution, use-pathsensitivity-none-for-file-inputs-and-pathsensitivity-relative-for-directories, use-unique-output-files-and-directories, don-t-hardcode-task-names-unless-they-are-documented-as-public-api, don-t-access-a-project-instance-during-task-execution, wire-lazy-task-outputs-using-map-and-flatmap
- `#up-to-date-checking` — avoid-dependson
- `#upgrades` — do-not-use-internal-apis, consider-use-of-incubating-apis-carefully, prefer-the-bin-gradle-distribution, don-t-hardcode-task-names-unless-they-are-documented-as-public-api
- `#version-catalog` — use-version-catalogs-to-centralize-dependency-versions, name-version-catalog-entries-appropriately
- `#wrapper` — prefer-the-bin-gradle-distribution, validate-the-gradle-distribution-sha-256-checksum, validate-the-gradle-wrapper-on-every-upgrade, do-not-run-gradlew-on-untrusted-projects