...
recorded. `docs/DESIGN.md` is the agreed design (5 milestones); **M1, the dynamic vertical slice, is done**, M2–M5
(Java extras/Flow/scrubber, static analysis, points-to, overlay) are not started. `docs/DESIGN.md` §12 records what M1
...
| `coroutree-gui` | Compose Desktop: `source/` (feeds, sessions), `view/` (testable logic), `ui/` (composables), `demo/` | per-OS uber jars published under classifiers |
...
classes are patched at load). New hook = one entry in `HookTable` + one method in `Hooks`.
